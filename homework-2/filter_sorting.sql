...
SELECT AVG(order_date) FROM orders where ship_country is 'Germany'; ?
...
SELECT min(unit_price) FROM products where discontinued != 1 and units_in_stock > 0;
SELECT max(unit_price) FROM products where discontinued != 1 and units_in_stock > 0;